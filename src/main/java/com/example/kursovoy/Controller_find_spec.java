...
public class Controller_find_spec {
    ...
    @FXML
    private TableColumn<Table_read_student, String> id_students;
    ...
    void initialize() {
        ...
        change.setOnAction(actionEvent ->
        {
            try {
                ...
                output.writeInt(7);
                output.writeInt(spec.getText().length());
                for(int i =0; i <spec.getText().length(); i++)
                {
                    output.writeChar(spec.getText().charAt(i));
                }
            }
            catch (Exception ex) {
            }
            id_students.setCellValueFactory(field -> new SimpleObjectProperty<>(field.getValue().getId_student()));
